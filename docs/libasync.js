...
addToLibrary({
...
#if ASYNCIFY == 2
      const promiseId = {{{ makeGetValue('newFiber', C_STRUCTS.emscripten_fiber_s.asyncify_data + C_STRUCTS.asyncify_data_s.rewind_id, 'i32') }}};
  ...
  emscripten_fiber_swap: (oldFiber, newFiber) => {
    ...
    let promiseId = {{{ makeGetValue('oldFiber', C_STRUCTS.emscripten_fiber_s.asyncify_data + C_STRUCTS.asyncify_data_s.rewind_id, 'i32') }}};
    if (!promiseId) {
      Fibers.topPromiseId++;
      promiseId = Fibers.topPromiseId;
    }

    {{{ makeSetValue('oldFiber', C_STRUCTS.emscripten_fiber_s.asyncify_data + C_STRUCTS.asyncify_data_s.rewind_id, 'promiseId', 'i32') }}};

    const promise = new Promise((resolve, reject) => {Fibers.resolvers[promiseId]=resolve;});
